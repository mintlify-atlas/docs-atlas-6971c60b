...
```cpp random_vector.cpp
template <typename T> vector<T> random_vector(int n, T from, T to);
```

Generates a vector of n random values in the range [from, to].
...
```cpp random_permutation.cpp
template <typename T> vector<T> random_permutation(int n, T start = 0);
```

Generates a random permutation of n elements starting from `start`.
...
```cpp std_library_find_nearest_set.cpp
template <typename T> T find_nearest(set<T> &st, T target);
```

Finds the element in the set nearest to the target value. If two elements are equidistant, returns the larger one.
